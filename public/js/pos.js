$(document).ready(function() {
    ...
    if ($('#search_product').length) {
        ...
        //For weighing_barcode
        var weighing_barcode='';
        //Add Product
          $('#search_product')
            .autocomplete({
                source: function(request, response) {
                    weighing_barcode=request.term;
                    var price_group = '';
                    var search_fields = [];
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed) {
                       ...
                       //Pre select lot number only if the searched term is same as the lot number
                        var purchase_line_id = ui.item.purchase_line_id && searched_term == ui.item.lot_number ? ui.item.purchase_line_id : null;
                         ...
                         pos_product_row(ui.item.variation_id, purchase_line_id,weighing_barcode);
                    } else {
   ...
   $('#customer_id').select2('close');
    ...
    $(window).resize(function() {
        ...
        /*$('div.product_list_body').css('min-height', div_height + 'px');
        $('div.pproduct_list_body').css('max-height', div_height + 'px');*/
    });
