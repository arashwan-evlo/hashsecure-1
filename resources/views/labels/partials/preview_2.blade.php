		...
		<td align="center" valign="center">
			<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in; justify-content: center;">
				<div style="font-family: Segoe UI Semibold;">
					...
					@if(!empty($print['category']))
						<b style="display: block !important; font-size: {{$print['category_size']}}px">{{$page_product->cat_name}}</b>
					@endif
					...
					{{-- Barcode --}}
					<img style="max-width:90% !important;height: {{$barcode_details->height*0.24}}in !important; display: block;margin-top: 5px" src="data:image/png;base64,{{DNS1D::getBarcodePNG($page_product->sub_sku, $page_product->barcode_type, 3,33, array(0, 0, 0), false)}}">
