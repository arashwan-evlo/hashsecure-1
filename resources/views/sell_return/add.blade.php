...
{!! Form::hidden('location_id', $sell->location->id, ['id' => 'location_id', 'data-receipt_printer_type' => $sell->location->receipt_printer_type ]); !!}
	...
	<div class="box box-solid">
		<div class="box-body">
			...
			<br>
				...
				<?php
					...
					<div class="payment_row">
						<div class="row">
							<div class="col-md-12 mb-12 hidden">
				...
				<span class="input-group-addon">
					<i class="fas fa-money-bill-alt"></i>
										...
										{!! Form::text("amount", '', ['class' => 'form-control payment-amount input_number', 'required', 'id' => "amount", 'placeholder' => __('sale.amount'), 'readonly' => $readonly]); !!}
									</div>
								</div>
							</div>
              ...
              <span class="input-group-addon">
                <i class="fa fa-calendar"></i>
											...
											{!! Form::text("paid_on",  @format_datetime('now'), ['class' => 'form-control paid_on', 'readonly', 'required']); !!}
										</div>
									</div>
								</div>
				...
				<span class="input-group-addon">
					<i class="fas fa-money-bill-alt"></i>
										...
										@if($readonly)
											{!! Form::hidden("method", $payment_line['method'], ['class' => 'payment_types_dropdown', 'required', 'id' => "method_$row_index"]); !!}
										@endif
									</div>
								</div>
							</div>
					...
					<span class="input-group-addon">
						<i class="fas fa-money-bill-alt"></i>
											...
											{!! Form::select("account_id", $accounts, !empty($payment_line['account_id']) ? $payment_line['account_id'] : '' , ['class' => 'form-control select2 account-dropdown', 'id' => !$readonly ? "account_$row_index" : "account_advance_$row_index", 'style' => 'width:100%;', 'disabled' => $readonly]); !!}
										</div>
									</div>
								</div>
							@endif
			...
			<div class="row">
				<div class="col-sm-12">
					<button type="submit" id="return-save" class="btn btn-primary pull-right">@lang('messages.save')</button>
				</div>
			</div>
		</div>
	</div>
