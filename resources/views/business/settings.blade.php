...
<script type="text/javascript">
    $(document).ready(function () {
        gentest();
        function gentest() {
            var codetemp =$('#t_label').val();
            var label_prefix = $('#label_prefix').val().length;
            var product_sku_length = $('#product_sku_length').val()*1;
            var qty_length = $('#qty_length').val()*1;
            var qty_length_decimal = $('#qty_length_decimal').val()*1;

            var barcode = codetemp.substr(0,product_sku_length);
            $('#t_barcode').val(barcode);


            var intval=codetemp.substr(product_sku_length,qty_length);


            var decimalval=codetemp.substr(product_sku_length+qty_length,qty_length_decimal);

            var weight=intval.concat('.',decimalval);
            $('#t_weiht').val(weight);
        }

        $(document).on('change','#qty_length_decimal ,#qty_length ,#label_prefix ,#product_sku_length ,#t_label',function () {
            gentest();
        });
    });
