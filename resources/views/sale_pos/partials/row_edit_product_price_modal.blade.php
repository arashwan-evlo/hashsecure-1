		...
		<div class="modal-body">
			...
			<div class="row">
				<div class="form-group col-xs-6 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
					<label>@lang('sale.unit_price_no_tax')</label>
                ...
                <div class="form-group col-xs-6 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    ...
                    <input type="text" name="products[{{$row_count}}][unit_price_inc_tax2]" class="form-control pos_unit_price_inc_tax input_number mousetrap" value="{{@num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->sell_price_inc_tax)}}" readonly>
                </div>
				...
				<div class="form-group col-xs-12 ">
					...
					{!! Form::select("products[$row_count][tax_id]", $tax_dropdown['tax_rates'], $tax_id, [ 'class' => 'form-control tax_id'], $tax_dropdown['attributes']); !!}
				</div>
				@if(!empty($warranties))
					<div class="form-group col-xs-12">
