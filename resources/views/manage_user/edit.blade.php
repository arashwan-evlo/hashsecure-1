...
<section class="content">
    ...
    <div class="row">
        ...
        {{-- Accounts 30-4-2025--}}
        <div class="col-md-12">
            @component('components.widget', ['title' =>'صلاحية الخزن'])
                <div class="row check_group">
                <div class="col-md-3">
                    <h4>عرض الخزن عند التحصيل و الدفع</h4>
                </div>
                <div class="col-md-9">
                    <div class="col-md-12">
                        <div class="checkbox">
                            <label>
                                {!! Form::checkbox('selected_accounts', 1, $user->selected_accounts,
                              ['class' => 'input-icheck  selected_accounts']); !!} السماح بخزن محددة للدفع و التحصيل
                            </label>

                        </div>
                    </div>
                    @foreach($accounts as $location)
                        <div class="col-md-12">
                            <div class="checkbox">
                                <label>
                                    {!! Form::checkbox('account_permissions[]', $location->id,is_array($permitted_accounts) && in_array($location->id, $permitted_accounts) ,
                                    [ 'class' => 'input-icheck account_permission']); !!} {{ $location->name }}
                                </label>
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="col-md-12">
            ...
            <div class="col-sm-4 selected_contacts_div @if(!$user->selected_contacts) hide @endif">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <div class="checkbox">
                            <br/>
                            <label>
                                {!! Form::checkbox('selected_department', 1,
                                $user->selected_department,
                                [ 'class' => 'input-icheck', 'id' => 'selected_department']); !!} {{ __( 'lang_v1.allow_selected_department' ) }}
                            </label>
                ...
                <div class="col-sm-4 selected_department_div @if(!$user->selected_department) hide @endif">
                    <div class="form-group">
                        {!! Form::label('selected_cdepartment', __('lang_v1.selected_department') . ':') !!}
                        <div class="form-group">
                            {!! Form::select('selected_department_ids[]', $department, $department_access, ['class' => 'form-control select2', 'multiple', 'style' => 'width: 100%;' ]); !!}
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#selected_contacts').on('ifUnchecked', function(event){
      ...
      $('#selected_department').on('ifChecked', function(event){
          $('div.selected_department_div').removeClass('hide');
      });
      $('#selected_department').on('ifUnchecked', function(event){
          $('div.selected_department_div').addClass('hide');
      });
    ...
    $('.account_permission').on('ifChecked',function (){

        $('.selected_accounts').iCheck('check');
      });
