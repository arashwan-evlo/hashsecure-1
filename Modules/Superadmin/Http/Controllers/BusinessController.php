...
class BusinessController extends BaseController
{
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', function($row) {
                    $html = '<a href="' . 
                            ...
                            if(request()->session()->get('user.business_id') != $row->id && $row->is_active ==0) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!empty($subscription_details['package_id']) && !empty($subscription_details['paid_via'])) {
                ...
                $subscription =  $this->_add_subscription($business->id, $package, $subscription_details['paid_via'], $subscription_details['payment_transaction_id'],$request->session()->get('user.id'), true);
            }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            if($business->is_active==1){
                $output = ['success' => 0,
                    'msg' =>'عفوا يجب إلغاء تنشيط الحساب أولا'
                ];
                return $output;
            };
            ...
            AccountTransaction::join('accounts','accounts.id','account_transactions.account_id')
                ->where('accounts.business_id', $id)->delete();
            AccountType::where('business_id', $id)->delete();
            AccountType::where('business_id', $id)->delete();
            Category::where('business_id', $id)->delete();
            Contact::where('business_id', $id)->delete();
            Discount::where('business_id', $id)->delete();


            Printer::where('business_id', $id)->delete();

            VariationLocationDetails::join('business_locations','business_locations.id',
                'variation_location_details.location_id')
                ->where('business_locations.business_id', $id)->delete();
            ProductVariation::join('products','products.id','product_variations.product_id')
                ->where('products.business_id', $id)->delete();

            PurchaseLine::join('transactions','transactions.id','purchase_lines.transaction_id')
                ->where('transactions.business_id', $id)->delete();
            TransactionSellLine::join('transactions','transactions.id','transaction_sell_lines.transaction_id')
                ->where('transactions.business_id', $id)->delete();
            TransactionPayment::join('transactions','transactions.id','transaction_payments.transaction_id')
                ->where('transactions.business_id', $id)->delete();

            TransactionSellLinesPurchaseLines::join('transaction_sell_lines','transaction_sell_lines.id','transaction_sell_lines_purchase_lines.sell_line_id')
            ...
            if (!empty($products_id)) {
                TransactionSellLine::whereIN('product_id', $products_id)->delete();
                VariationLocationDetails::whereIn('product_id', $products_id)->delete();
                Variation::whereIn('product_id', $products_id)->delete();
            }
            Product::where('business_id', $id)->delete();
            CashRegisterTransaction::join('cash_registers','cash_registers.id'
                ,'cash_register_transactions.cash_register_id')
                ->where('cash_registers.business_id', $id)->delete();


            CashRegister::where('business_id', $id)->delete();
            Role::where('business_id', $id)->delete();

            Transaction::join('business_locations','business_locations.id',
                'transactions.location_id')->where('transactions.business_id', $id)->delete();

            Transaction::where('business_id', $id)->delete();
            BusinessLocation::where('business_id', $id)->delete();

            InvoiceScheme::where('business_id', $id)->delete();
            InvoiceLayout::where('business_id', $id)->delete();

            Business::where('id', $id)->delete();
            User::where('business_id', $id)->delete();
            $users = DB::table('activity_log')
                ->where('business_id', $id)->delete();
    ...
    public function changebusiness(Request $request){
        ...
        /*$users=Business::select('business.name','users.name','users.id','first_name','lastname','surname')
          ->join('users','users.busines_id','business.id')->get();*/
