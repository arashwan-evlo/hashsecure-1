...
class ProductionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            if(!empty($request->start_date)){
                $productions->whereBetween('transaction_date',[$request->start_date,$request->end_date]);
            }

            if(!empty($request->mfg_is_final)){
                $productions->where('mfg_is_final',$request->mfg_is_final);
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($request->input('production_cost_type')==0){
                $transaction_data['mfg_production_cost'] = $this->productUtil->num_uf($request->input('production_cost'));
                $transaction_data['mfg_production_cost_type'] ='percentage';

            }else{
                $transaction_data['mfg_production_cost'] = $this->productUtil->num_uf($request->input('production_cost'))*$quantity;
                $transaction_data['mfg_production_cost_type'] ='constant';
