...
class RecipeController extends Controller
{
    ...
    {
        $business_id = request()->session()->get('user.business_id');
        if (!($this->moduleUtil->hasThePermissionInSubscription($business_id, 'manufacturing_module')) || !auth()->user()->can('manufacturing.add_recipe')) {
            abort(403, 'Unauthorized action.');
        }

        try {
            $input = $request->only(['variation_id', 'ingredients', 'total', 'instructions',
                'ingredients_cost', 'waste_percent', 'total_quantity', 'extra_cost','extra_cost_type']);
            if (!empty($input['ingredients'])) {
                ...
                $recipe = MfgRecipe::updateOrCreate(
                    ...
                    [
                        ...
                        'extra_cost_type' => $input['extra_cost_type'],
