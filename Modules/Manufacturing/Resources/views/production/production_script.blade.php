<script type="text/javascript">
	...
	$(document).on('change', '#production_cost, select.sub_unit, input.mfg_waste_percent,#extra_cost,#extra_cost_type', function(){
		calculateRecipeTotal();
	});
	...
	function calculateRecipeTotal() {
        ...
        var production_cost_type=$('#extra_cost_type').val();
        var production_cost =  __calculate_amount('percentage', production_cost_percent, total_ingredients_cost);
        if(production_cost_type==1){
            production_cost=production_cost_percent;
        }
        total_ingredients_cost += production_cost;


        $('span#total_production_cost').text(__currency_trans_from_en(production_cost, true));

        var final_price = total_ingredients_cost;
