...
class DataController extends Controller
{
    ...
    public function user_permissions()
    {
        return [
            [
                'value' => 'essentials.essentials_modul',
                'label' => __('essentials::lang.essentials_modul'),
                'default' => false
            ],
            [
    ...
    public function modifyAdminMenu()
    {
        ...
        if (false) {
          ...
          Menu::modify('admin-sidebar-menu', function ($menu) {
                    ...
                    $menu->dropdown(
                        ...
                        function ($sub) {
                          /*  $sub->url(
                            ...
                            $sub->url(
                                action('\Modules\Essentials\Http\Controllers\DashboardController@employee'),
                                __('essentials::lang.employees'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'employee']
                            );

                          $sub->url(
                                action('\Modules\Essentials\Http\Controllers\AttendanceController@shift_attendance_list'),
                                'دفتر الحضور',
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'employee']
                            );
                            ...
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'leave-type']
                            );

                            $sub->url(
                                action('\Modules\Essentials\Http\Controllers\EssentialsLeaveController@index'),
                                __('essentials::lang.leave'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'leave']
                            );
                            $sub->url(
                                action('\Modules\Essentials\Http\Controllers\AttendanceController@index'),
                                __('essentials::lang.attendance'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'attendance']
                            );
                            ...
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'essentials' && request()->segment(2) == 'allowance-deduction']
                            );

                            $sub->url(
                                action('\Modules\Essentials\Http\Controllers\PayrollController@index'),
                                __('essentials::lang.payroll'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'payroll']
                            );

                            $sub->url(
                                action('\Modules\Essentials\Http\Controllers\EssentialsHolidayController@index'),
                                __('essentials::lang.holiday'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'holiday']
                            );

                            $sub->url(
                                action('TaxonomyController@index', 'type=hrm_department'),
                                __('essentials::lang.departments'),
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'taxonomies']
                            );
                            ...
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-users', 'active' => request()->segment(1) == 'hrm' && request()->segment(2) == 'settings']
                            );
                ...
                if (auth()->user()->can('essentials.essentials_modul')) {
                    $menu->url(
        ...
        if ($is_essentials_enabled) {
            Menu::modify('admin-sidebar-menu', function ($menu) {
                $menu->url('https://hr.prosofteg.com/',
                    __('essentials::lang.hrm'),
                    ['icon' => 'fa fas fa-wrench', 'active' => request()->segment(1) == 'test']
                )
                    ->order(87);
            });
