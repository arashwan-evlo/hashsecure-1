    ...
    public function parse_notification($notification)
    {
        ...
        if ($notification->type == 'Modules\Project\Notifications\NewProjectAssignedNotification') {
            ...
            if (!empty($project)) {
                ...
                $link = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]);
            ...
            if (!empty($task)) {
                ...
                $link = action('\Modules\Project\Http\Controllers\ProjectController@show', [$task->project_id]);
    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_project_enabled) {
            if(auth()->user()->can('project.create_project')){
                Menu::modify('admin-sidebar-menu',function ($menu) use ($is_admin) {
                    $menu->dropdown(
                        ...
                        function ($sub){
                            $sub->url(
                                ...
                                ['icon' => 'fa fa-project-diagram', 'active' =>request()->segment(1) == 'project' ]
                            );

                            $sub->url(
                                action('\Modules\Project\Http\Controllers\TaskController@index'),
                                __('project::lang.my_tasks'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'project']
                            );
                            ...
                            $sub->url(
                                action('\Modules\Project\Http\Controllers\ReportController@index'),
                                __('report.reports'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'taxonomies']
                            );
