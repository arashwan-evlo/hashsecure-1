...
class ProjectController extends Controller
{
                    ...
                    foreach ($sorted_projects as $key => $projects) {
                        ...
                        foreach ($projects as $project) {
                            ...
                            if (auth()->user()->can('project.edit_project')) {
                                $edit = action('\Modules\Project\Http\Controllers\ProjectController@edit', [$project->id]);
                            }

                            $delete = '';
                            if (auth()->user()->can('project.delete_project')) {
                                $delete = action('\Modules\Project\Http\Controllers\ProjectController@destroy', [$project->id]);
                            }

                            $view = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]);

                            $overviewTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=overview';

                            $activitiesTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=activities';

                            $taskTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=project_task';

                            $timeLogTabUrl = '';
                            if (isset($project->settings['enable_timelog']) && $project->settings['enable_timelog']) {
                                $timeLogTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=time_log';
                            }

                            $docNoteTabUrl = '';
                            if (isset($project->settings['enable_notes_documents']) && $project->settings['enable_notes_documents']) {
                                $docNoteTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=documents_and_notes';
                            }
                            ...
                            if ((isset($project->settings['enable_invoice']) && $project->settings['enable_invoice']) && ($is_lead || $is_admin)) {
                                $invoiceTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=project_invoices';
                            }

                            $settingsTabUrl = '';
                            if ($is_lead || $is_admin) {
                                $settingsTabUrl = action('\Modules\Project\Http\Controllers\ProjectController@show', [$project->id]).'?view=project_settings';
                            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!empty($project_members['attached'])) {
                ...
                $project['link'] = action('\Modules\Project\Http\Controllers\ProjectController@show', [ $project->id]);
    ...
    public function postSettings(Request $request)
    {
        ...
        return redirect()->action(
            '\Modules\Project\Http\Controllers\ProjectController@show',[$project_id])->with('status', $output);
    }
