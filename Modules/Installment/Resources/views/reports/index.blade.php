                              ...
                              <div class="table-responsive">
                                <table class="table table-bordered table-striped " id="installments_table">
                                    ...
                                    <tr>
                                        ...
                                        <th style="width: 60px">رقم القسط </th>
                                        ...
                                        <th style="width: 140px">قيمة مسددة </th>
                                        <th>أيام التأخير</th>
                                        <th>تاريخ السداد</th>
...
<script>
    $(document).ready(function () {
        installments_table= $('#installments_table').DataTable({
            ...
            serverSide: true,
            ajax:{
                url:"{{action('\Modules\Installment\Http\Controllers\InstallmentController@instalments')}}" ,
                data:function(d) {
            ...
            columns: [
                { data: 'id', name: 'installments.id' , searchable: false},
                { data: 'installment_number', name: 'installments.installment_number' },
                { data: 'name', name: 'contacts.name' },
                { data: 'installmentdate', name: 'installments.installmentdate'},
                { data: 'installment_value', name: 'installments.installment_value'},
                { data: 'benefit_value', name: 'installments.benefit_value'},
                { data: 'total_value', name: 'total_value', searchable: false},
                { data: 'paid_value', name: 'paid_value' },
                { data: 'paid_status', name: 'paid_status' },
                { data: 'paid_date', name: 'installments.paid_date' },
            ],
        });
    ...
    installments_table.ajax.reload();
        ...
        $(document).on('click', '.installmentdelete', function (e) {
            e.preventDefault();
        swal({
            title: LANG.sure,
        ...
        $(document).on('click', '.paymentdelete', function (e) {
            e.preventDefault();
            swal({
                title: LANG.sure,
                text: 'سوف يتم حذف القسط ',
