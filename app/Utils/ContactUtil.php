...
class ContactUtil extends Util
{
    ...
    public function createNewContact($input)
    {
        ...
        if ($count == 0) {
            ...
            if (!empty($opening_balance)) {
                ...
                $ob_transaction=$transactionUtil->createOpeningBalanceTransaction($contact->business_id, $contact->id, $opening_balance, $contact->created_by, false);
            }
    ...
    public function updateContact($input, $id, $business_id)
    {
        ...
        if ($count == 0) {
            ...
            $opening_balance_type=$input['opening_balance_type'];
            if (isset($input['opening_balance_type'])) {
                unset($input['opening_balance_type']);
            }

            $opening_balance_account_id=$input['opening_balance_account_id'];
            if (isset($input['opening_balance_account_id'])) {
                unset($input['opening_balance_account_id']);
            }
            ...
            if (!empty($ob_transaction)) {
                ...
                /* please don't change opening balance Ali 23-2-2025*/
                ...
                if (!empty($opening_balance)) {
                    $ob_transaction=  $transactionUtil->createOpeningBalanceTransaction($business_id, $contact->id, $opening_balance, $contact->created_by, false);
