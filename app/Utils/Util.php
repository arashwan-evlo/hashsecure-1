...
class Util
{
    ...
    public function num_uf($input_number, $currency_details = null)
    {
        ...
        /* added by Ali 21-02-2025 */
        $num = preg_replace('/[^\d]+(?=\d{3}(?:\.\d+)?$)|[^\d.]++/', '', $input_number);
    ...
    public function num_f($input_number, $add_symbol = false, $business_details = null, $is_quantity = false)
    {
        ...
        if ($is_quantity) {
            $currency_precision = session('business.quantity_precision', 2);
        }
    ...
    */
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        if(!empty($custom_labels['payments']['custom_pay_1']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_2']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_3']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_4']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_5']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_6']))
        ...
        if(!empty($custom_labels['payments']['custom_pay_7']))
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($request->$file_name->getSize() <= config('constants.document_size_limit')) {
               /* $new_file_name = time() . '_' . $request->$file_name->getClientOriginalName();*/
                $new_file_name = Str::uuid() . '.'. $request->$file_name->getClientOriginalExtension();
    ...
    */
    ...
    public function number_un_format($number)
    {
        $unformatted = preg_replace('/[^\d.]/', '', $number);
        return $unformatted;
