...
class ModuleUtil extends Util
{
    ...
    public function getModulePermission($function_name, $arguments = null)
    {
        $modules = Module::toCollection()->toArray();
        ...
        foreach ($modules as $module => $details) {
            if ($this->isModuleInstalled($details['name'])) {
                /* add by ali to view only Module in packeg*/
                if($this->IsmoduleInpackeg($details['alias']))
                    $installed_modules[] = $details;
            }
        }
        ...
        if (!empty($installed_modules)) {
            foreach ($installed_modules as $module) {
                ...
                if (class_exists($class)) {
                    $class_object = new $class();
                    $dd= $this->hasThePermissionInSubscription($business_id,$module['name']."_module");
                    if (method_exists($class_object, $function_name)) {
                        $data[$module['name']] = call_user_func([$class_object, $function_name]);
                    }
                }
    ...
    public function IsmoduleInpackeg($modulename){
        $modules = Module::toCollection()->toArray();
        $date_today = \Carbon::today();
        $business_id = request()->session()->get('user.business_id');

        $package =Business::leftjoin('subscriptions AS s', function($join) use ($date_today){
        ...
        if(!empty($package['package_details'])){
            $collection=(json_decode($package['package_details'],true));

            $exists= array_key_exists($modulename.'_module',$collection);
            return $exists;
        }
