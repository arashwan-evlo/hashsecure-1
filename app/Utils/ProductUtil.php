...
class ProductUtil extends Util
{
    ...
    /* For sub locations*/
    public function SublocationdecreaseQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        $qty_difference = $new_quantity - $old_quantity;
        $product = Product::find($product_id);
        ...
        if ($product->enable_stock == 1) {
            //Decrement Quantity in variations location table
            $details = LocatonGroupVariation::where('variation_id', $variation_id)
            ...
            if (empty($details)) {
                $variation = Variation::find($variation_id);
                $details = LocatonGroupVariation::create([
            ...
            $details->decrement('quantity', $qty_difference);
        }

        return true;
    }

    public function SublocationincreaseQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        $qty_difference = $new_quantity - $old_quantity;
        $product = Product::find($product_id);
        ...
        if ($product->enable_stock == 1) {
            ...
            $details->increment('quantity', $qty_difference);
        }
    ...
    public function getProductStockDetails($business_id, $filters, $for,$alert_qty=false)
    {
        ...
        $products = $query->select(
           ...
           DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) FROM transactions
                  ...
                  AND (pl.variation_id=variations.id)) as stock_price2"),

            /*Add by eng ali to correct price in report old is above*/
            DB::raw(" SUM(vld.qty_available*variations.dpp_inc_tax)  as stock_price"),
...
$stock_history = Transaction::
    ...
    ->whereIn('transactions.type', ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase','production_sell', 'purchase_return', 'sell_return','stocking'])
    ->select(
...
->get();
        ...
        foreach ($stock_history as $stock_line) {
            if ($stock_line->transaction_type == 'sell') {
            ...
            }elseif ($stock_line->transaction_type == 'stocking') {
                ...
                $stock_history_array[] = [
                    ...
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'purchase_transfer',
                    'type_label' => __('lang_v1.stocking'),
                    'ref_no' => $stock_line->invoice_no,
                    'transaction_id' => $stock_line->transaction_id
                ];
            }
