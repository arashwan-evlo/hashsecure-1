...
class TransactionUtil extends Util
{
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'sub_location_id'=>! empty($product['sub_location_id']) ?$product['sub_location_id']:0,
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1)
    {
        ...
        $sell_line->fill(['product_id' => $product['product_id'],
            ...
            'sub_unit_id' => !empty($product['sub_unit_id']) ? $product['sub_unit_id'] : null,
            'res_service_staff_id' => !empty($product['res_service_staff_id']) ? $product['res_service_staff_id'] : null,
            'sub_location_id' => !empty($product['sub_location_id']) ? $product['sub_location_id'] : null,
        ]);
        $sell_line->save();
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        $payment_type='credit';
        if($transaction->type==='purchase' || $transaction->type==='sell_return'  || $transaction->type==='expense'  ){
            $payment_type='debit';
        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output = [
            ...
            'commercial_register' =>trim($il->commercial_register),
        ...
        $output['show_customer'] =$il->show_customer;
        ...
        $output['shipping_charges'] = !empty($transaction->shipping_charges) ? $transaction->shipping_charges : 0.0;
        $output['shipping_details'] = !empty($transaction->shipping_details) ? $transaction->shipping_details : '';
        $output['shipping_status'] = !empty($transaction->shipping_status) ? $transaction->shipping_status :'';

        $output['show_shipping']=$il->show_shipping;
       ...
       /* dd( $output['invoice_heading']);*/
        ...
        $output['taxed_subtotal_uf'] = $taxed_subtotal;
        ...
        $output['total_discount_uf']=$total_line_discount+$discount;
    ...
    */
    public function getSellsLast30Days($business_id, $group_by_location = false)
    {
        ...
        $query->select(
            ...
            DB::raw("SUM(IF(transactions.type='sell', final_total,-1*final_total) ) as total_sells"))
        ...
        $sells = $query->whereIn('transactions.type', ['sell','sell_return'])->get();
...
//  $string = $table_name . "quantity_sold + " . $table_name . "quantity_adjusted + " . $table_name . "quantity_returned + " . $table_name . "mfg_quantity_used";
    ...
    public function createOpeningBalanceTransaction($business_id, $contact_id, $amount, $created_by, $uf_data = true)
    {
        ...
        $ob_data = [
                    ...
                    'created_by' => $created_by,
                     'additional_notes'=>'رصيد إفتتاحي عملاء و موردين'
                ];
    ...
    */
    ...
    public function getGrossProfit_old($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = TransactionSellLine::join('transaction as sale', 'transaction_sell_lines.transaction_id', '=', 'sale.id')
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $gross_profit_obj = $query->select(DB::raw('SUM( 
                        (transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) * (SL.unit_price_inc_tax - IFNULL(PL.purchase_price_inc_tax, v.dpp_inc_tax) ) ) as gross_profit'))
            ->first();
    ...
    */
    ...
    /* Function added by eng Mohamed Ali 18-1-2022*/

    /* function to get all sell discount transaction and lines*/
    public function getselldiscount($business_id,$user_id=null,$open_time= null,$close_time= null){
      ...
      if(!empty($open_time) && !empty($close_time ))
          $query->whereBetween('created_at', [$open_time, $close_time]);

      $query->addselect(DB::raw("SUM(discount_value) as transaction_discount"));

      $transaction_discount=$query->first()->transaction_discount;

   /*Return discount */
      $query=Transaction::where('transactions.type','sell_return')
          ->where('transactions.status','final')
          ->where('transactions.business_id',$business_id);
      if(!empty($user_id))
          $query->where('transactions.created_by',$user_id);
      if(!empty($open_time) && !empty($close_time ))
          $query->whereBetween('updated_at', [$open_time, $close_time]);
        $query->addselect(DB::raw("SUM(discount_value) as transaction_discount"));

      $output['transaction_discount']=$transaction_discount-$query->first()->transaction_discount;
      /*Get Line discount */
      $query=Transaction::where('transactions.type','sell')
          ->where('transactions.status','final')
          ->where('transactions.business_id',$business_id);
      if(!empty($user_id))
          $query->where('transactions.created_by',$user_id);
      if(!empty($open_time) && !empty($close_time ))
          $query->whereBetween('transactions.updated_at', [$open_time, $close_time]);

          $query->join('transaction_sell_lines','transactions.id','=','transaction_sell_lines.transaction_id');
          $query->addselect(DB::raw("SUM(IF(line_discount_type='percentage',line_discount_amount*unit_price_before_discount*(quantity-quantity_returned)/100,line_discount_amount*(quantity-quantity_returned))) as line_discount"));

      $output['line_discount']=$query->first()->line_discount;

      return $output;
  }
  //
    public function getsell_line_discount(
        $business_id,
        $transaction_types,
        $start_date = null,
        $end_date = null,
        ...
        if (!empty($start_date) && !empty($end_date) && empty($cashRegisterSession)) {
            $query->whereBetween(DB::raw('date(transaction_date)'), [$start_date, $end_date]);
        }
        if (empty($start_date) && !empty($end_date) && empty($cashRegisterSession)) {
            $query->whereDate('transaction_date', '<=', $end_date);
        }
        if( !empty($cashRegisterSession) ){
            $query->whereBetween(DB::raw('transaction_date'), [$start_date, $end_date]);
        }
        //Filter by the location
        if (!empty($location_id)) {
            $query->where('transactions.location_id', $location_id);
        ...
        if (in_array('sell_return', $transaction_types)) {
         ...
         /*discount= IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0))*/
        if (in_array('sell', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='sell' AND transactions.status='final',line_discount_value,0))  as line_discount_value"));
        }

        $transaction_totals = $query->first();

        $line_discount_value= !empty($transaction_totals->line_discount_value) ?
                              $transaction_totals->line_discount_value : 0;


        return $line_discount_value;
    }


/* Function to get all sell transaction count grouped by type*/
    public function getsellcount($business_id,$user_id=null,$open_time= null,$close_time= null){
        $query=Transaction::where('transactions.status','final')
                           ->where('transactions.business_id',$business_id);
        if(!empty($user_id))
        ...
        $query->addselect(DB::raw("count(IF(transactions.type='sell',1,null)) as sell "));
        $query->addselect(DB::raw("count(IF(transactions.type='sell_return',1,null)) as sell_return "));
        $query->addselect(DB::raw("count(IF(transactions.type='expense',1,null)) as expense "));
        $query->addselect(DB::raw("count(IF(transactions.type='expense_refund',1,null)) as expense_refund "));
      

        /* $query->addselect('type',DB::raw("count(*) as count "))->groupBy('type');*/
         $data=$query->first();
        $output['sell']=$data->sell;
        $output['sell_return']=$data->sell_return;
        $output['expense']=$data->expense;
        $output['expense_refund']=$data->expense_refund;
        return $output;
    }

    /* Function to get sell tax*/
    public function getselltax($business_id,$user_id=null,$open_time= null,$close_time= null){
        $query=Transaction::where('transactions.type','sell')
            ->where('transactions.status','final')
            ->where('transactions.business_id',$business_id);
        if(!empty($user_id))
        ...
        $query->addselect(DB::raw("SUM(tax_amount) as transaction_tax"));
        $transaction_tax=$query->first()->transaction_tax;
        ...
        $output['transaction_tax']=$transaction_tax - $query->first()->transaction_tax;
        ...
        $query->addselect(DB::raw("SUM(item_tax) as line_tax"));

        $output['line_tax']=$query->first()->line_tax;


        return $output;
    }

   public function writelog($line=null){
    $date=Carbon::now();
   // file log in public folder
