...
class User extends Authenticatable
{
  ...
  public function departmentAccess()
    {
        return $this->belongsToMany(\App\Category::class, 'user_category_access');
    }
    ...
    public function permitted_accounts()
    {
        $user = $this;

        if ($user->can('access_all_accounts')) {
            return 'all';
        } else {
            $business_id = request()->session()->get('user.business_id');
            $permitted_accounts = [];
            ...
            foreach ($accounts as $account) {
                if ($user->can('account.' . $account->id)) {
                    $permitted_accounts[] = $account->id;
                }
            }

            return $permitted_accounts;
        }
    }
