...
class ManageUserController extends Controller
{
    ...
    public function create()
    {
       ...
       $accounts= Account::where('business_id', $business_id)
        ...
        return view('manage_user.create')
                ->with(compact('roles', 'username_ext', 'contacts', 'locations', 'form_partials','accounts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $user_details = $request->only(['surname', 'first_name', 'last_name',
                ...
                'max_sales_discount_percent', 'family_number', 'alt_number','selected_accounts'
            ]);
            ...
            if (!isset($user_details['selected_accounts'])) {
                $user_details['selected_accounts'] = false;
            }
    ...
    public function edit($id)
    {
        ...
        $demo_usernames = config('constants.demo_usernames');
        if(in_array($username,explode(',', $demo_usernames))){
            abort(403, 'Unauthorized action.');
        }
        ...
        $department_access = $user->departmentAccess->pluck('id')->toArray();


        $contacts = Contact::contactDropdown($business_id, true, false);

        $department=Category::forDropdown($business_id,'hrm_department');
        ...
        $permitted_accounts=UserAccountAccess::where('user_id',$user->id)->where('status',1)->pluck('account_id')->toArray();;

        return view('manage_user.edit')
                ->with(compact('roles', 'user', 'contact_access', 'contacts', 'is_checked_checkbox',
                    'locations', 'permitted_locations', 'form_partials', 'username_ext',
                'department','department_access','accounts','permitted_accounts'
                ));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $user_data = $request->only(['surname', 'first_name', 'last_name', 'email', 'selected_contacts', 'marital_status',
                ...
                'selected_department','selected_accounts'
            ...
            if (!isset($user_data['selected_accounts'])) {
                $user_data['selected_accounts'] = 0;
            }
    ...
    private function giveAccountPermissions($user, $request)
    {
        ...
        $user_id=$user->id;
        UserAccountAccess::where('user_id',$user_id)->update(['status'=>0]);
        if(!empty($account_permissions)) {
            foreach ($account_permissions as $account) {
                UserAccountAccess::updateorcreate(
                    [
                        'user_id' => $user_id,
                        'account_id' => $account
                    ], ['user_id' => $user_id,
