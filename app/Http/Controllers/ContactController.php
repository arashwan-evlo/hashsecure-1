...
class ContactController extends Controller
{
    ...
    private function indexCustomer()
    {
        ...
        $contacts = Datatables::of($query)
            ...
            ->addColumn(
                'due',
                '<span class="contact_due" data-orig-value="{{$total_invoice - $invoice_received+$opening_balance }}" data-highlight=true>@format_currency($total_invoice - $invoice_received+ $opening_balance )</span>'
            )
            ->addColumn(
                'return_due',
    ...
    public function create()
    {
        ...
        $journal=new AccountTransaction();



        return view('contact.create')
            ->with(compact('types', 'customer_groups', 'selected_type',
                'module_form_parts', 'users','accounts','sub_accounts','journal'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $input = $request->only(['type', 'supplier_business_name',
                ...
                'pay_term_number', 'pay_term_type', 'mobile', 'landline',
                'alternate_number', 'city', 'state', 'country', 'address_line_1',
                ...
                'account_id','opening_balance_type','opening_balance_account_id'
                 ]);
        ...
        if($request->routing_type==='main_account'){
            if(empty($input['contact_id'])){
                $output = ['success' => false,
                    'msg' =>'عفوا برجاء إدخال كود الحساب'
                ];
                return $output;
            }
            ...
            if(empty($request->account_id)){
                $output = ['success' => false,
                    'msg' =>'عفوا برجاء إختيار الحساب الرئيسي'
                ];
            ...
            /* TODO: add Account Transaction*/

            if(!empty($input['opening_balance']) && $input['opening_balance']>0 ) {

                $contact = $output['data'];
                $ob_transaction = $output['ob_transaction'];
                $data = [
                    'business_id' => $business_id,
                    ...
                    'note' => 'رصيد إفتتاحي',
                    'type' => $input['opening_balance_type'],
                    'sub_type' => 'opening_balance',
                    'amount' => $ob_transaction->final_total,
                    'transaction_id' => $ob_transaction->id,
                    'operation_date' => $ob_transaction->transaction_date,
                ];

                $debit = AccountTransaction::createAccountTransaction($data);
                if ($input['opening_balance_type'] === 'debit') {
                    $data['type'] = 'credit';
                } else {
                    $data['type'] = 'debit';
                }

                $data['account_id'] = $input['opening_balance_account_id'];
    ...
    public function show($id)
    {
        ...
        if(!in_array($contact->id, $user_contacts) && Auth()->user()['selected_contacts']){
            abort(403, 'Unauthorized action.');
        }
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            $contact = Contact::where('business_id', $business_id)->find($id);
             ...
             $ContactAccess=$this->ContactAccess($id);
             if(!$ContactAccess){
                   return view('contact.partials.contact_not_allowed');
             }
            ...
            $ob_transaction =  Transaction::where('contact_id', $id)
                                            ...
                                            ->first();
            $journal=new AccountTransaction();
            $opening_balance_account=new AccountTransaction();
            if(!empty($ob_transaction)){
                $journal=AccountTransaction::where('transaction_id',$ob_transaction->id)
                    ->where('account_id',$contact->account_id)->first();

                $opening_balance_account=AccountTransaction::where('transaction_id',$ob_transaction->id)
                    ->where('account_id','!=',$contact->account_id)->first();
            }
            ...
            $accounts=Account::forDropdownmainaccount($business_id,false);
            $sub_accounts=Account::forDropdownAllsubaccount($business_id,false);


            return view('contact.edit')
                ->with(compact('contact', 'types', 'customer_groups', 'opening_balance',
                    'users','accounts','sub_accounts','journal','opening_balance_account'));
        }
    }

    public function ContactAccess($contact_id)
    {
       if(auth()->user()['selected_contacts']){
           $user_contacts = auth()->user()->contactAccess->pluck('id')->toArray();
           if(!in_array($contact_id, $user_contacts) ){
               return false;
           }
       }
           return true;
        ...
        if (request()->ajax()) {
            try {
                ...
                $input = $request->only(['type', 'supplier_business_name', 'prefix', 'first_name', 'middle_name', 'last_name', 'tax_number', 'pay_term_number', 'pay_term_type', 'mobile', 'address_line_1', 'address_line_2', 'zip_code', 'dob', 'alternate_number', 'city', 'state', 'country', 'landline', 'customer_group_id', 'contact_id', 'custom_field1', 'custom_field2', 'custom_field3', 'custom_field4', 'custom_field5', 'custom_field6', 'custom_field7', 'custom_field8', 'custom_field9', 'custom_field10', 'email', 'shipping_address', 'position', 'shipping_custom_field_details', 'export_custom_field_1', 'export_custom_field_2', 'export_custom_field_3', 'export_custom_field_4', 'export_custom_field_5',
                    'export_custom_field_6', 'assigned_to_users','price_group_id','commercial_register','account_id',
                    'opening_balance_type','opening_balance_account_id']);


                $business_id = request()->session()->get('user.business_id');
             /* ToDO: check if account_code exist */
                $name_array = [];
            ...
            try {
                ...
                if ($count == 0) {
                    ...
                    if (!$contact->is_default) {
                        ...
                        $contact->delete();
                        $transactions=Transaction::where('business_id', $business_id)->where( 'contact_id',$id)->first();
                       if(!empty($transactions)){
                           $transaction_id=$transactions->id;
                           AccountTransaction::deleteAccountTransaction($transaction_id);
                       }


                        /* TODO: Delete account for contact */
    ...
    public function getLedger()
    {
        ...
        $default_datetime = $this->businessUtil->format_date('now', false);

        $ledger_details['end_date']= $default_datetime;
        ...
        if (request()->input('action') == 'pdf') {
            ...
            if ($format == 'format_2') {
                $html = view('contact.ledger_format_2')
                        ->with(compact('ledger_details', 'contact', 'for_pdf', 'location','business'))->render();
                        
            } else {
             //   dd($business,$location);
        ...
        if ($format == 'format_2') {
            return view('contact.ledger_format_2')
             ->with(compact('ledger_details', 'contact', 'location','business'));
        } else {
    ...
    public function updateStatus($id)
    {
        ...
        if (request()->ajax()) {
            ...
            if(!$ContactAccess){
                ...
                $output = ['success' => false,
                    'msg' => __("lang_v1.conatct_not_allowaed")
                ];
                return $output;
            }
            $business_id = request()->session()->get('user.business_id');
...
$user=User::all();
       return   collect([$user])->downloadExcel(
            'product.xlsx',
            null,
            true
        );
