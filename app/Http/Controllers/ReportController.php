...
//dd($filters);
    ...
    public function getproductSellReport(Request $request)
    {
       ...
       /* if (!auth()->user()->can('purchase_n_sell_report.view') || !auth()->user()->can('access_sell_return')) {
            abort(403, 'Unauthorized action.');
        }*/
...
//sell for return
 public function getproductSellReturnReport(Request $request)
    ...
    public function getProfitby_product(Request $request)
    {
        $business_id = request()->session()->get('user.business_id');
        $start_date = $request->get('start_date');
        $end_date = $request->get('end_date');
        ...
        $query = TransactionSellLinesPurchaseLines::
             join('transaction_sell_lines as SL', 'SL.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
            ...
            ->where('sale.business_id', $business_id)
             ->select('p.name as product','unit_price_inc_tax','p.id','sale.id as saleid',
                 DB::raw('SUM((transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) * (SL.unit_price_inc_tax - IFNULL(PL.purchase_price_inc_tax, v.dpp_inc_tax) ) ) as gross_profit')
              )->groupby('saleid','p.id');
